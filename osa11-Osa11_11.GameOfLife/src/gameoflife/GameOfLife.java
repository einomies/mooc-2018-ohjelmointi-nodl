    ...
    public void kehity() {
        ...
        for (int x = 0; x < this.taulukko.length; x++) {
            for (int y = 0; y < this.taulukko[x].length; y++) {
                int solunArvo = this.taulukko[x][y];
                kopio[x][y] = this.taulukko[x][y];
                int elossaLkm = elossaOleviaNaapureita(this.taulukko, x, y);
//                tulostaSolunTilanne(x, y, solunArvo, elossaLkm);
                if (solunArvo == 1) {
                    if (elossaLkm < 2 || elossaLkm >= 4) {
                        kopio[x][y] = 0;
                    }
                } else if (solunArvo == 0 && elossaLkm == 3) {
                    kopio[x][y] = 1;
                }
            }
        }
...
//        int korkeus = taulukko[x].length;
        for (int xMuunnos = -1; xMuunnos <= 1; xMuunnos++) {
            for (int yMuunnos = -1; yMuunnos <= 1; yMuunnos++) {
                if (x + xMuunnos < 0 || y + yMuunnos < 0) {
                    continue;
                }
                if (x + xMuunnos > taulukko.length - 1 || y + yMuunnos > taulukko[x].length - 1) {
                    continue;
                }
    ...
    private void tulostaSolunTilanne(int x, int y, int solunArvo, int elossaLkm) {
        System.out.println("x=" + x + " y=" + y + " solunArvo=" + solunArvo + " elossaLkm=" + elossaLkm);
    }
}
